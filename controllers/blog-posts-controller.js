...
const getBlogPosts = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError("We were unable to gather blog posts.", 500);
    return next(error);
  }
  if (!blogPosts) {
...
const getBlogPost = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError("We were unable to find this blog post.", 500);
    return next(error);
  }
  // If there was no blogPost found
  if (!blogPost) {
    const error = new HttpError(
      `Could not find blog post with Id ${blogPostId}`,
      404
    );
    return next(error);
  }
...
const updateBlogPost = async (req, res, next) => {
  try {
    ...
    console.log(err);
    const error = new HttpError(
      "We were unable to update this blog post.",
      500
    );
    return next(error);
  }
...
const deleteBlogPost = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError("We were unable to delete this blog post", 500);
    return next(error);
  }
  if (!result) {
    const error = new HttpError("Could not find blog post with ID", 404);
    return next(error);
  }
  res.json(result);
};
